...
export TEP, load_tep, abrv_to_full, get_tep_location, get_graph_location,
       get_mutual_info_location, get_sample_location, load_graph,
       generate_mutual_info, load_mutual_info, load_or_generate_mutual_info,
       sample, sample_with_dt, sample_at_ts, store_sample, store_sample_p
...
function get_mutual_info_location(obj::TEPLocation, dt::Float64)::String
    return "$(obj.root)/mim-$(obj.network_type)-$(obj.graph_id)-$(obj.tep_id)-$(@sprintf("%.2f", dt)).npz"
end
get_mutual_info_location(obj::TEP, dt::Float64)::String = get_mutual_info_location(obj.location, dt)
...
# function generate_mutual_info(obj::TEP, dt::Float64; store::Bool=true)::Array{Float64,2}
#     samples = sample_with_dt(obj, dt)
#     M = mutual_information_matrix(samples)
#     if store
#         filename = get_mutual_info_location(obj, dt)
#         Numpy.savez(filename, M=M)
#     end
#     return M
# end

# function load_mutual_info(obj::TEP, dt::Float64)::Array{Float64,2}
#     filename = get_mutual_info_location(obj, dt)
#     return load(filename)["M"]
# end

# function load_or_generate_mutual_info(obj::TEP, dt::Float64)::Array{Float64,2}
#     path = Path(get_mutual_info_location(obj, dt))
#     if isfile(path)
#         try
#             return load_mutual_info(obj, dt)
#         catch
#             # Failed to load, generate instead
#         end
#     end
#     return generate_mutual_info(obj, dt)
# end
